...
const { isValidEmail, isCharNumOnly, isNumOnly, isCharOnly, isValidDate, isValidMinLength, isValidMaxLength, isCharAndSpaceOnly } = require('../../utils/validation');
const { runPreparedQuery, fetchQuery } = require('../../database');
const { getUserByEmail, getUserByUsername, insertAdministrator, getCountActiveAdmin } = require('../../queries/admin/auth');
...
router.post('/register', async (req, res) => {
   ...
   form.parse(req, async (err, fields, files) => {
        ...
        if (!isCharAndSpaceOnly(Firstname)) {
            flag = false;
            message.push("Firstname must contain letters only.")
        }
        if (!isCharAndSpaceOnly(Middlename)) {
            flag = false;
            message.push("Middlename must contain letters only.")
        }
        if (!isCharAndSpaceOnly(Lastname)) {
            flag = false;
            message.push("Lastname must contain letters only.")
        }
        if (!isValidEmail(Email)) {
            flag = false;
            message.push("Email must be a valid email address")
        }
        if (!isNumOnly(ContactNumber)) {
            flag = false;
            message.push("Contact Number must contain number only.")
        }
        if (!Address) {
            flag = false;
                ...
                if (err) {
                    ...
                    try {
                        ...
                        if (flag) {
                            ...
                            let respinsertAdministrator = await runPreparedQuery(insertAdministrator, 
                                { 
                                    Firstname,
                                    Middlename,
                                    Lastname,
                            ...
                            if (respinsertAdministrator.rowsAffected > 0) {
...
router.get('/count', fetchQuery(getCountActiveAdmin));
